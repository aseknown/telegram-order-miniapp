...
export async function management(context,user,helpers) {
  ...
  if(path.startsWith('/admin')) {
    ...
    if(path==='/admin/permissions' && request.method==='GET') return json({admins:(await env.DB.prepare('SELECT c.id,c.name,c.phone,a.created_at FROM platform_admins a JOIN customers c ON c.id=a.customer_id ORDER BY a.created_at LIMIT 100').all()).results,
      roles:[{name:'Customer',permissions:'Own orders, receipts, sessions and support conversations'},{name:'Shop owner',permissions:'Own shop profile, catalog, customers and order review'},{name:'Platform admin',permissions:'Shop verification, plans, support, investigations and role management'}]});
    if(path==='/admin/permissions' && request.method==='PATCH') {
      const body=await bodyJson(request);
      const phone=required(body.phone,'Verified account phone',16);
      if(!/^\+[1-9]\d{7,14}$/.test(phone) || typeof body.admin!=='boolean' || typeof body.expected_admin!=='boolean') throw new ApiError(400,'Provide an international phone and the expected/current role.');
      const target=await env.DB.prepare('SELECT id FROM customers WHERE phone=?').bind(phone).first();
      if(!target) throw new ApiError(404,'No verified account uses that phone.');
      const note=required(body.note,'Permission change reason',500);
      const event=audit('customer',target.id,'PLATFORM_ROLE_CHANGED',{admin:body.admin,note},'(EXISTS (SELECT 1 FROM platform_admins WHERE customer_id=?))=?',[target.id,body.expected_admin?1:0]);
      const mutation=body.admin
        ? env.DB.prepare('INSERT INTO platform_admins (customer_id,created_at) SELECT ?,? WHERE EXISTS (SELECT 1 FROM audit_events WHERE id=?) ON CONFLICT(customer_id) DO NOTHING').bind(target.id,now(),event.id)
        : env.DB.prepare('DELETE FROM platform_admins WHERE customer_id=? AND EXISTS (SELECT 1 FROM audit_events WHERE id=?)').bind(target.id,event.id);
      const statements=[event.statement,mutation];
      if(!body.admin) statements.push(env.DB.prepare('UPDATE auth_sessions SET revoked_at=? WHERE customer_id=? AND revoked_at IS NULL AND EXISTS (SELECT 1 FROM audit_events WHERE id=?)').bind(now(),target.id,event.id));
      const results=await env.DB.batch(statements);
      if(!results[0].meta.changes) throw new ApiError(409,'This role changed. Refresh before trying again.');
      return json({ok:true});
    }
