...
export function slugValue(value) {
  if (typeof value !== 'string' || !/^[a-z0-9][a-z0-9_-]{2,39}$/.test(value) || ['api','admin','merchant','assets','shop','www','pricing','support'].includes(value)) {
...
async function customer(context, validate, verified = true) {
  ...
  const auth = await validate(header.startsWith('Telegram ') ? header.slice(9) : '', context.env.BOT_TOKEN);
  if (!auth.ok || !Number.isSafeInteger(auth.user?.id) || auth.user.id <= 0) throw new ApiError(401, 'Open the shop in Telegram to sign in.');
  const user = await context.env.DB.prepare('SELECT * FROM customers WHERE telegram_id=?').bind(String(auth.user.id)).first();
  if (!user && verified) throw new ApiError(403, 'Share your own phone number with the bot, then refresh your account.');
  return { user, telegram: auth.user };
}
...
export async function marketplace(context, validate) {
  ...
  try {
    ...
    if (publicMatch && request.method === 'GET') {
      ...
      if (!shop) throw new ApiError(404, 'Shop not found.');
      const category=optional(url.searchParams.get('category') || '','Category',80);
      const { results } = await env.DB.prepare("SELECT id,name,description,price_minor,category_id FROM available_products WHERE shop_id=? AND (?='' OR category_id=?) ORDER BY slot LIMIT 50 OFFSET ?").bind(shop.id,category,category,pageOffset(url)).all();
      return json({ shop, products: results,categories:(await env.DB.prepare('SELECT id,name FROM categories WHERE shop_id=? ORDER BY name LIMIT 100').bind(shop.id).all()).results });
    }
    ...
    if (merchant) {
      ...
      if (section === 'products' && request.method === 'POST' && !resource) {
        ...
        await env.DB.prepare('INSERT INTO products (id,shop_id,name,description,price_minor,active,category_id) VALUES (?,?,?,?,?,?,?)').bind(productId,shop.id,...fields,category).run();
      ...
      if (section === 'products' && request.method === 'PATCH' && resource) {
        ...
        if(Object.keys(body).length===1 && Object.hasOwn(body,'category_id')) {
          const result=await env.DB.prepare('UPDATE products SET category_id=? WHERE id=? AND shop_id=?').bind(category,resource,shop.id).run();
          if(!result.meta.changes) throw new ApiError(404,'Product not found.'); return json({ok:true});
        }
        const fields = productFields(body);
        const changed = await env.DB.prepare('UPDATE products SET name=?,description=?,price_minor=?,active=?,category_id=?,archived=0 WHERE id=? AND shop_id=? AND source_id IS NULL').bind(...fields,category,resource,shop.id).run();
